      ...
      if (PageStatus1 == VALID_PAGE) /* Page0 receive, Page1 valid */
      {
        ...
        for (VarIdx = 0; VarIdx < NumbOfVar; VarIdx++)
        {
          ...
          if (VarIdx != x)
          {
            ...
            ReadStatus = EE_FLASH_ReadVariable(VirtAddVarTab[VarIdx], &DataVar);
...
static uint16_t EE_VerifyPageFullWriteVariable(uint16_t VirtAddress, uint16_t Data)
{
  ...
  uint32_t PageEndAddress = EEPROM_START_ADDRESS + 0x7FF;
...
{
  FLASH_Status FlashStatus = FLASH_COMPLETE;
  uint32_t NewPageAddress = EEPROM_START_ADDRESS + 0x3FF, OldPageAddress = EEPROM_START_ADDRESS;
